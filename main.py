...
class md6hash():
	# Преобразует массив байтов в список 64-битных слов
	...
	# Преобразует список 64-битных слов в массив байтов
	...
	# Обрезает массив байтов до указанного размера
	...
	def __bytes(self, i_str):
		i_str = binascii.hexlify(i_str.encode("utf-8"))
		o_byte = [int(i_str[i:i+2], 16) for i in range(0, len(i_str), 2)]

		return o_byte

	# Подготавливает данные для хеширования, преобразуя их в байты и проверяя,
	# находятся ли входные параметры в допустимых пределах
	...
	# Вычисляет хеш MD6 заданных данных в виде шестнадцатеричной строки
	...
	#  Вычисляет хеш MD6 заданных данных в виде строки сырых байтов
